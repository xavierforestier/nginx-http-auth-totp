...
http_code=$( curl -sio /tmp/last -w "%{http_code}" http://localhost:8080/public )
[ "${http_code}" == "404" ] || echo -e "\033[1;31mFAILED\033[0m (expected 404, but got ${http_code})" && cat /tmp/last
...
[ "${http_code}" == "401" ] || echo -e "\033[1;31mFAILED\033[0m (expected 401, but got ${http_code})" && cat /tmp/last
